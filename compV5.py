...
def GridSearch_KK_dtpeak_HM(argv = None):
    ...
    def get_lnprob(k, dtpeak):
        ...
        for mt in MtotalList_ini:
            ...
            power_vec = psd(freqs)
            Oxt22 = calculate_Overlap_tmp(EOBModes_C.get_mode(2,2).value, NRModes_C.get_mode(2,2).value, power_vec, df, NFFT)
            Oxt21 = calculate_Overlap_tmp(EOBModes_C.get_mode(2,1).value, NRModes_C.get_mode(2,1).value, power_vec, df, NFFT)
            Oxt33 = calculate_Overlap_tmp(EOBModes_C.get_mode(3,3).value, NRModes_C.get_mode(3,3).value, power_vec, df, NFFT)
            Oxt44 = calculate_Overlap_tmp(EOBModes_C.get_mode(4,4).value, NRModes_C.get_mode(4,4).value, power_vec, df, NFFT)
